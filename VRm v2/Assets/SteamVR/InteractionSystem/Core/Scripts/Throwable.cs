...
namespace Valve.VR.InteractionSystem
{
	...
	public class Throwable : MonoBehaviour
	{
        ...
        string throw_audioPath = "Audio/throwSound";
        string collide_audioPath;
        protected AudioClip throwClip, collideClip;
        protected float defaultPitch;
        protected float minimumSpeed = 0.1f;
        protected float audioVolume;
        protected bool isGlass;
        ...
        protected virtual void Awake()
		{
            ...
            //Set up the audio sources for all throwable objects in the scene
            audioSource = gameObject.AddComponent<AudioSource>();
            throwClip = Resources.Load<AudioClip>(throw_audioPath);
            audioSource.clip = throwClip;

            if(gameObject.name.Contains("Flask"))
            {
                Debug.Log("Flask Found");
                collide_audioPath = "Audio/glassBounce";
                audioVolume = .02f;
                isGlass = true;
            }
            else
            {
                collide_audioPath = "Audio/thud";

                audioVolume = .005f;
                isGlass = false;
            }

            collideClip = Resources.Load<AudioClip>(collide_audioPath);
        ...
        private void OnCollisionEnter(Collision collision)
        {
            if (isGlass)
            {
                audioSource.pitch = Random.Range(1f, 2.0f);
            }
            else
            {
